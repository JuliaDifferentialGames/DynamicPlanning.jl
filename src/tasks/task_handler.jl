...
function task_handler(problem)
    ...
    problem_queue = Vector{PriorityQueue()}
    problem_dict = Vector{Dict()}
    ...
    for robot ∈ problem.workspace.robots
        ...
        for (task_id, task) ∈ enumerate(robot.tasks)
            ...
            push!(pq, task_id => task.priority)

            # Create a problem from the task
            if task isa NavigationTask
                # Make a TPBVP 
                prob = TPBVP(robot.n, robot.m, robot.dynamics, robot.x0, task.goal, (0.0, task.deadline), 10) # Might need to change x0, timespan, and N
